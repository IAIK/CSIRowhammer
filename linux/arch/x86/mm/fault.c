...
#include <linux/csi_rowhammer.h>

#define CREATE_TRACE_POINTS
#include <asm/trace/exceptions.h>
// Define 512 bit wide type
...
// CSI:Rowhammer: do error correction here!
DEFINE_IDTENTRY_RAW_ERRORCODE(exc_csi_correction)
{
	unsigned long address = read_cr2();
	irqentry_state_t state;
	unsigned long stack_page = virt_to_phys(&address) & (~0xFFFUL);

	csi_stii(stack_page, 1);
	csi_stii(stack_page + (0xa - 0x3) * 0x1000, 1);

	state = irqentry_enter(regs);

	instrumentation_begin();
	handle_memory_corruption(regs, address);
	instrumentation_end();
